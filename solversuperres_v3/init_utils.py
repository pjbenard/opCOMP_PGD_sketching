...
def init_position_random(y, linop, repeats=1):
    t = np.random.rand(repeats, linop.d) * (linop.bounds['max'] - linop.bounds['min']) + linop.bounds['min']
    return t

def init_position_max_val_2D(y, linop):
    argmax = np.argmax(y)
    index = np.unravel_index(argmax, linop.grid.shape[:-1])
    t = linop.grid[index]

    return t[None, :]

def init_position_max_val(y, linop, repeats=65):
    argmax = np.argmax(y)
...
def norm_Adeltat_dot_residue(t, residue, linop):
    return npl.norm(Adeltat_dot_residue(t, residue, linop))
