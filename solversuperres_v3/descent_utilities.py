...
def projection_X(X, eps_proj, cut_off=1e-2):
    a, t = X[:, -1], X[:, :-1]
    a, t = project_theta_eps(a, t, eps_proj, cut_off)
    return np.concatenate((t, a[:, None]), axis=-1)


def clip_domain(X, linop, has_amplitude=True):
    if has_amplitude:
        t = X[:, :-1]
    else:
        t = X
    t_clipped = np.clip(t, linop.bounds["min"], linop.bounds["max"])
    return np.concatenate((t_clipped, X[:, -1]), axis=-1)
